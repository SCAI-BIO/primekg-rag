...
def map_questions_to_nodes():
    ...
    try:
        ...
        node_collection = node_client.get_collection(name=NODE_COLLECTION_NAME, embedding_function=embedding_fn)
        
        mappings_client = chromadb.PersistentClient(path=str(MAPPINGS_DB_PATH))
        mappings_collection = mappings_client.get_or_create_collection(name=MAPPINGS_COLLECTION_NAME)

    except Exception as e:
        logger.critical(f"Could not connect to ChromaDB: {e}")
        return None
        
    results = node_collection.query(query_texts=processed_questions, n_results=1, include=["documents", "metadatas", "distances"])
    all_mappings = []
    if results:
        for i, q in enumerate(questions):
            if results['documents'] and len(results['documents']) > i and results['documents'][i]:
                ...
                all_mappings.append({'original_question': q, 'best_match_node_name': doc, 'similarity_score': f"{similarity:.4f}", 'node_id': meta.get('id'), 'node_type': meta.get('type')})
    if not all_mappings:
...
def extract_subgraphs_for_nodes(nodes_to_process: list):
    ...
    for node_name in tqdm(nodes_to_process, desc="Extracting Subgraphs"):
        try:
            ...
            subgraph_df = duckdb.query(f"""SELECT * FROM read_csv_auto('{KG_CSV_PATH}', ignore_errors=true) WHERE x_name = '{safe_node_name}' OR y_name = '{safe_node_name}';""").to_df()
            if not subgraph_df.empty:
                ...
                for col in ['id', 'name', 'type', 'source']:
                    ...
                    if x_col in subgraph_df.columns and y_col in subgraph_df.columns:
                        x_orig, y_orig = subgraph_df.loc[needs_swap, x_col].copy(), subgraph_df.loc[needs_swap, y_col].copy()
            ...
            continue
    logger.info(f"(SUCCESS) Stage 2 complete. Subgraph files created in '{SUBGRAPH_OUTPUT_DIR}'.")
