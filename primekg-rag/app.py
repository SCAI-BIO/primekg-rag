...
NODE_COLORS = {
    ...
    "phenotype": "#00BCD4", "pathway": "#FFC107", "molecular_function": "#F44336",
    "biological_process": "#E91E63", "cellular_component": "#673AB7",
    "compound": "#8BC34A", "chemical_compound": "#8BC34A",
    "biological_entity": "#FFEB3B", "exposure": "#FF9800",
    "symptom": "#CDDC39", "default": "#607D8B",
}
...
st.markdown("Select a topic to load its knowledge subgraph, then ask any question about it.")
...
if not subgraph_files:
    ...
    selected_type_filter = st.selectbox("Filter subgraphs by node type:", options=filter_options)

    if selected_type_filter != "All Types":
        filtered_files = [f for f in subgraph_files if node_type_map.get(Path(f).stem.replace('_subgraph', '').replace('_', ' ')) == selected_type_filter]
    ...
    if not subgraphs_to_display:
        ...
        selected_file = st.selectbox("Select a Subgraph to Analyze:", options=subgraphs_to_display, key="subgraph_selector")
        ...
        if selected_file:
            ...
            tab1, tab2, tab3, tab4 = st.tabs(["AI Expert Analysis", "Chat with AI Analyst", "Interactive Graph", "Raw Data"])
                ...
                if prompt := st.chat_input("Ask a follow-up question..."):
                    ...
                    with st.chat_message("assistant"):
                        with st.spinner("Thinking..."):
                            ...
                            response = generate_chat_response(context, prompt, st.session_state.messages[:-1])
            ...
            with tab3:
                ...
                if not st.session_state.current_subgraph_df.empty:
                    ...
                    for _, row in st.session_state.current_subgraph_df.head(MAX_NODES).iterrows():
                        src, src_type, dst, dst_type, rel = row["x_name"], row["x_type"], row["y_name"], row["y_type"], row["display_relation"]
                        src_color, dst_color = NODE_COLORS.get(src_type, NODE_COLORS["default"]), NODE_COLORS.get(dst_type, NODE_COLORS["default"])
